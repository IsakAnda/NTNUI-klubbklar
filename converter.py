...
def converter(order: pd.DataFrame, filename):
    ...
    with pd.ExcelWriter(filename, engine='xlsxwriter') as writer:
        styled_df.to_excel(writer, sheet_name='sheet1', index=False, startrow=1)
        ...
        for i in range(len(order['Lag'])):
            if order['Lag'].to_list()[i] != last_team:
                ...
                if 'Totalt per lag' in order.columns:
                    worksheet.merge_range(last_row+2, 7, i+1, 7, f'=SUMMER(g{last_row+2}:g{i+1})', person_formats[teams.index(order['Lag'].to_list()[i-1])%2])
                    #worksheet.write_formula(last_row+2, 7, f'=SUMMER(g{last_row+2}:g{i+1})')
                    #worksheet.merge_range(last_row+2, 7, i+1, 7, "", person_formats[teams.index(order['Lag'].to_list()[i-1])%2])
        ...
        if 'Totalt per lag' in order.columns:
            #worksheet.write_formula(last_row+2, 7, f'=SUMMER(g{last_row+2}:g{len(order["Lag"])+1})')
            #worksheet.merge_range(last_row+2, 7, len(order['Lag'])+1, 7, '', person_formats[(len(teams)-1)%2])
            worksheet.merge_range(last_row+2, 7, len(order['Lag'])+1, 7, f'=SUMMER(g{last_row+2}:g{len(order["Lag"])+1})', person_formats[(len(teams)-1)%2])        # FIXME writes =@SUMMER
        ...
        for i in range(len(order['Navn'])):
            if order['Navn'].to_list()[i] != last_person:
                ...
                if 'Kostnad enkelt personer' in order.columns:
                    worksheet.merge_range(last_row+2, 6, i+1, 6, f'=SUMMER(f{last_row+2}:f{i+1})', person_formats[teams.index(order['Lag'].to_list()[i-1])%2])
        ...
        if 'Kostnad enkelt personer' in order.columns:
            worksheet.merge_range(last_row+2, 6, len(order['Navn'])+1, 6, f'=SUMMER(f{last_row+2}:f{len(order["Navn"])+1})', person_formats[(len(teams)-1)%2])